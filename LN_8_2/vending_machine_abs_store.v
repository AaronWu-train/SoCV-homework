module vending_machine (
    ...
    // Abstraction 1: store next-state is unconstrained (over-approximation)
    input  [2:0]  free_next_store50,
    input  [2:0]  free_next_store10,
    input  [2:0]  free_next_store5,
    input  [2:0]  free_next_store1,
    ...
    always @(*) begin
        ...
        // Yosys rejects procedural while in combinational always; unroll with fixed bound (avail* <= 7).
        if (enough_money) begin
            for (k = 0; k < 7; k = k + 1) begin
                if ((remain >= 50) && (use50 < avail50)) begin
                    remain = remain - 50;
                    use50  = use50 + 3'd1;
                end
            end
            for (k = 0; k < 7; k = k + 1) begin
                if ((remain >= 10) && (use10 < avail10)) begin
                    remain = remain - 10;
                    use10  = use10 + 3'd1;
                end
            end
            for (k = 0; k < 7; k = k + 1) begin
                if ((remain >= 5) && (use5 < avail5)) begin
                    remain = remain - 5;
                    use5   = use5 + 3'd1;
                end
            end
            for (k = 0; k < 7; k = k + 1) begin
                if ((remain >= 1) && (use1 < avail1)) begin
                    remain = remain - 1;
                    use1   = use1 + 3'd1;
                end
            end
    ...
    always @(*) begin
        ...
        next_store50 = free_next_store50;
        next_store10 = free_next_store10;
        next_store5  = free_next_store5;
        next_store1  = free_next_store1;
    ...
    // -------------------------------------------------
    // BDD monitors (bad conditions)
    // We will prove AG(~bad_xxx)
    // -------------------------------------------------
